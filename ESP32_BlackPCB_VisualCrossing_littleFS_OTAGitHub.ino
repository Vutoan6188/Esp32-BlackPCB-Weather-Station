...
const char* FIRMWARE_FILENAME = "Esp32_BlackPCB_Weather_Station.ino.bin";  // Tên file .bin trong phần release
...
void checkFirmwareUpdate() {
  ...
  if (httpCode == 200) {
    ...
    if (latestVersion != FIRMWARE_VERSION) {
      ...
      tft.drawString("HTTP code: " + String(code), 0, 270);
      if (code == 200) {
        ...
        if (Update.begin(len)) {
          ...
          if (written == len) {
            ...
            tft.drawString("Firmware write incomplete — missing data! (%d/%d bytes)\n" + String(written, len), 0, 240);
          }
          if (Update.end()) {
