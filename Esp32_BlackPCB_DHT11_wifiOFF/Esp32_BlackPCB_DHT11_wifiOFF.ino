...
int pwmPin = 2; //39 for S2

#include <DHT.h>
#define DHTPIN 22 //5 for S2
...
void setup() {
  ...
  tft.setRotation(2); // (0) for Esp32DevKit/S2miniILI9341
...
// Check if we should update weather information
  if (booted || (m == 0 && !GetWeatherData)) { // || m == 30)
    wifiON();
    updateWeatherData(h, m);
    syncTime();
    if (booted || (h == 0 && !GetAstronomy)) {
